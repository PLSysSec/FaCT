  ...
  let fenv' = Env.new_fenv() in
    Hashtbl.iter
      (fun name { Env.f_rvt; Env.f_args } ->
         let name' = "_" ^ name in
         let ctx_lt = {
           Cast.ty=Cast.BoolMask;
           Cast.lbl=Cast.Public;
           Cast.kind=Cast.Val
         } in
         let fentry = { Cast.f_rvt=transform_vt f_rvt;
                        Cast.f_args=ctx_lt :: List.map transform_lt f_args }
...
and transform_stm' rty venv mem ctx stm =
  ...
  let rec transform_arg = fun { Pos.data } ->
    ...
    let base_e =
      match e with
        ...
        | Tast.TCallExp(n,args) ->
          let ctx_arg = Cast.ValArg c in
          let args' = ctx_arg :: List.map transform_arg args in
            Cast.CallExp("_" ^ n, args')
  ...
  let transform_block ctx b =
    ...
    let stms' = List.flatten(List.map (transform_stm rty venv' mem' ctx) b.body) in
      { Cast.venv=venv'; Cast.mem=mem'; Cast.body=stms' }
  in
  ...
  | Tast.TIf(e,bt,bf) ->
    ...
    let mnot = Cast.Assign(tname,b_and (b_not e') c) in

    let m = make_expr (Cast.VarExp tname) Cast.BoolMask in
    let ctx' = Context(m) in
...
and transform_fdec { Pos.data } =
  let transform_fdec' = function
    | { Tast.t_name=name; Tast.t_params=args; Tast.t_rvt; Tast.t_body=body } ->
      ...
      let ctx_arg = { Cast.name="__ctx"; Cast.lt=ctx_lt } in
      let args' = ctx_arg :: List.map transform_param args in
      ...
      let body'' = { Cast.venv=venv'; Cast.mem=mem'; Cast.body=[rval]@[rset]@body' } in
        Cast.FunctionDec("_" ^ name, args', rvt', body'',
