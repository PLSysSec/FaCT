...
let ret = Return(Primitive(Number 1))
let printf' = VarDec("x", Int, CallExp("printf", [Primitive(ByteArray("vaevaerwgr\n"))]))
let printf'' = VarDec("xx", Int, CallExp("printf", [Primitive(ByteArray("vavaergreevaerwgr\n"))]))
let dec = VarDec("x", Int, Primitive(Number 666))
let assign = Assign("x", Primitive(Number 10))
let printi = VarDec("i", Int, CallExp("printf", [Primitive(Number(666))]))
...
let _ =
  ...
  | [|_;"run"|] -> run_command "lli" [|"lli"; "out.ll"|]
  | [|_;"link"|] -> run_command "llvm-as" [|"llvm-as"; "out.ll"|]
  | [|_;"assemble"|] -> run_command "llc" [|"llc"; "out.bc"|]
  | [|_;"share"|] -> run_command "clang" [|"clang"; "-c"; "out.s"|]
  | [|_;"harness"|] -> run_command "gcc" [|"gcc"; "-o"; "final"; "harness.o"; "out.o"|]
