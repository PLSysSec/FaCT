...
let size_of_lexpr = function
  | LIntLiteral n -> n
  | LVariable _ -> raise CodegenError
  | LLength _ -> raise CodegenError
            ...
            begin
              match lexpr.data with
                ...
                | LVariable var_name ->
                  ...
                  build_load arr "loadlvararray" builder (*  TODO: HMMMMM??? *)
                | LLength var_name ->
                  let arr = find_var venv var_name in
                  build_load arr "loadllenarray" builder (*  TODO: HMMMMM??? *)
            end
          | ArrayCopy var_name ->
            let arr = find_var venv var_name in
